...
const ExploreMore = () => {
  ...
  return (
    <div>
      ...
      {/* Sub-Heading text */}
      <p className='text-center text-brownred-50  text-lg font-semibold mt-3 mb-3 lg:mb-0 '>
        Learn to build anything you can imagine
      </p>  

      {/*Tabs div */}
      <div className=' hidden lg:flex mt-5 shadow-custom  flex-row rounded-full
       bg-brownred-900 mb-5 border-brownred-300
      p-1'>
      {
        tabsName.map( (element, index) => {
            return (
                ...
                ${currentTab === element 
                ? "bg-brownred-950 text-richblack-5 font-medium"
                : "text-richblack-200" } rounded-full transition-all duration-200 cursor-pointer
                hover:bg-brownred-950 hover:text-richblack-5 px-8 py-2`}
                key={index}
